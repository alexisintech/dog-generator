...
fetch(BREEDS_URL)
    ...
    .then(data => {
        ...
        for (let i = 0; i < breedsArray.length; i++) {
            ...
            option.value = breedsArray[i]; //<option value='breed'> the array's index value (the index of the specific breed) at that point of the loop
            option.innerText = breedsArray[i]; //<option value='breed'>breed</option> adding the text of the breed to be seen in the drop down box and the for loop does this for each array item (each breed)
            select.appendChild(option); //append child is taking whatever html element you specify, in this case, "select" which looks for .breeds the drop-down selector box, 
                                        //and we're adding a nested element in it. so its creating another <option> and the for loop will do this for each array item (each breed)
        }
    })
...
const doggoInfo = {
    ...
    assignMF() {
        if (Math.floor(Math.random() * 2) == 0) { // Math.random() * 2 gives us a number between 0-2. Math.floor gives us the closest whole number.
            this.MF = "Female"; //"this" is the array, MF is the property we set earlier on line 45 "MF: '',"
            this.assignName(this.fNames); //"this" is the array, "assignName()" is a method we create later on line 64 in this array, and we are passing the parameter this.fNames which is an array on line 39
            console.log('true result');
        } else {
            this.MF = "Male";
            this.assignName(this.mNames);
        }
        document.getElementById("MF").innerHTML = `Sex: ${this.MF}`; //Changing the text of our #MF to show either male or female depending on what our conditional just found
    },

    assignName(array) {
        this.rname = array[Math.floor(Math.random()*array.length)]; //this function is saying we'll take in a parameter that is an array. and with that array, we will get the index of (a random whole number multiplied by the array length)
                                                                    // and assign whatever element is in that index to rname (which should be a name from either fNames or mNames because of assignMF() on line 52)
        document.getElementById("dog-name").innerHTML = `${this.rname}` //we display that chosen name in the appropriate element that we grab from the HTML #dog-name
    },
    ...
    //Fisher-Yates Shuffle - shuffling an array to grab a random property from it
    ...
    assignLikes() {
        this.likes = this.yatesShuffle(this.likesList).slice(0,2); //we set this.likes by calling Yates Shuffle, pass this.likesList through the Yates Shuffle, and we grab the first two values from it using slice.
                                                                   // remember that slice will grab indexes from the first index u choose up to the second index but not including that index number (so 0-2 will be o, 1, and not including 2)
        document.getElementById("likes").innerHTML = `Likes: ${this.likes[0]}, ${this.likes[1]}`; //we display those two values in the #likes element from HTML
    },
